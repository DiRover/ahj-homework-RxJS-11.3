...
var Store = /*#__PURE__*/function () {
  ...
  _createClass(Store, [{
    ...
    value: function doneTask(id) {
      ...
      // метод для установки/снятия галочки (задача выполнена/невыполнена)
      this.currerntProject.tasks.map(function (item) {
        // ищем задачу по айди (именно этот блок кода не нужен, т.к. он меняет значение только в текущем проекте, но не в общем списке проектов и задач)
      ...
      this.data.forEach(function (item) {
        // делаем всё тоже самое что описано выше, только меняем статус выполнения задачи
        if (item.name === _this.currerntProject) {
          // в общем списке проектов и задач
  ...
  RenderTask_createClass(RenderTask, [{
    ...
    value: function init() {
      ...
      this.store.state$.subscribe(function (data) {
        // получаем объект с проектными задачами и текущим, выбранным, проектом
        _this.render(data);
    ...
    value: function render(data) {
      // метод для рендера списка
      var projects = data.projects; // получаем проекты со списками задач из объекта

      var currerntProject = data.currerntProject; // получаем текщий, выбранный, проект
      ...
      elemCon.innerHTML = "<div class=\"title-container\"><h2 class=\"title\">Task</h2><div>Project:</div><div class=\"current-project\">".concat(currerntProject.name, "</div></div>"); // сделали заголовок с текущим названием выбранного проекта

      projects.forEach(function (item) {
        // обрабатываем массив объектов с проектами
        if (item.name === currerntProject.name) {
          // если название проекта совпадает с названием выбранного проекта
          item.tasks.forEach(function (i) {
            // то начинаем обрабатывать массив объектов с задачами выбранного проекта
            var elem = document.createElement('div'); // создаём объект для задачи

            elem.setAttribute('class', 'task'); // прописываем ему классы стилей

            elem.setAttribute('id', i.id); // присваиваем ему айди задачи

            var visible = i.done ? 'visible' : 'hidden'; // если задача выполнена, галочка видна, если нет, то скрыта
    ...
    value: function renderStateList(data) {
      ...
      var nameProjects = data.projects.map(function (item) {
        return item.name;
      }); // создаём массив названий проектов, можно и по другому, это лишнее

      nameProjects.forEach(function (item) {
        // обрабатываем массив с названиями проектов
        var elem = document.createElement('div'); // создаём элемент для хранения названия проекта
...
var RenderState = /*#__PURE__*/function () {
  ...
  RenderState_createClass(RenderState, [{
    ...
    value: function init() {
      ...
      .pipe(pluck('projects') // оператором pluck ловим в нём объект с проектами по ключу projects
    ...
    value: function render(projects) {
      ...
      projects.forEach(function (item) {
        ...
        var doneTask = item.tasks.filter(function (i) {
          // получаем массив выполненных задач, для отображения их числа
          if (i.done) return i;
        });
        var elem = document.createElement('div'); // создаём элемент для названия проекта и числа выполненных задач
        ...
        elem.innerHTML = "<div>".concat(item.name, "</div><div>").concat(doneTask.length, "</div>"); // прописываем имя и кол-во задач
...
function handler(target, store) {
  ...
  var stateList = document.querySelector('.state-list'); // поп ап со списком проектов, просто здесь его удобней получать

  if (target.classList.contains('current-project')) {
    // поп ап появляется при выборе текущего проекта, для отобрадения задач данного проекта
    ...
    // кликнули на другом проекте в поп апе
    stateList.classList.toggle('hidden'); // закрываем поп ап

    var project = target.textContent; // получаем название нового текщего проекта для отображения задач
    ...
    // кликнули на пустой кружок, чтоб галочку поставить, тип задача выполнена
    var id = target.id; // получаем айди задачи, прописан у самого элемента, по которому кликнули

    store.doneTask(id); // отправляем айди в обработчик, метод экземпляра класса Store
  } else if (target.classList.contains('check')) {
    // кликнули на галочку, тип задача всё-таки не выполнена
    var _target$closest = target.closest('.cycle'),
        _id = _target$closest.id; // получаем айди задачи, прописан у самого элемента, по которому кликнули


    store.doneTask(_id); // отправляем айди в обработчик, метод экземпляра класса Store
  } else if (!stateList.classList.contains('hidden')) {
    // клик по любому месту страницы
...
var page = document.querySelector('.page'); // получаем корневой элемент для отрисовки приложения
